...
function plotUniqueLanguages() {
    const svg = d3.select("#unique_plot");
    svg.selectAll("*").remove();
    const w = +svg.attr("width"), h = +svg.attr("height");
    const x = d3.scaleLinear().domain([0, unique_history.length - 1]).range([40, w - 10]);
    const y = d3.scaleLinear().domain([0, d3.max(unique_history) || 1]).range([h - 30, 10]);
    const line = d3.line()
        .x((d, i) => x(i))
        .y(d => y(d));
    svg.append("g").attr("transform", `translate(0,${h - 30})`)
    ...
    svg.append("text")
        .attr("x", w / 2).attr("y", 20)
        .attr("text-anchor", "middle")
        .text("Number of Unique Languages Over Time");
}
...
function plotPCA() {
    ...
    svg.append("text")
        ...
        .attr("y", 20)
        .attr("text-anchor", "middle")
        .text("UMAP of Languages (2D projection)");
}
...
d3.select("#restart").on("click", () => {
    ...
    plotFitness();
    plotLanguages();
    unique_history = []; // Reset history
    plotUniqueLanguages(); // Clear plot
    plotPCA();
});
