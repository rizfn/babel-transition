...
def plot_lattice_umap_clusters(L, g, a, r, B, mu, K):
    lattice_file = f"src/understandabilityVsHamming2D/outputs/latticeNbrVsGlobal/L_{L}_g_{g}_a_{a}_r_{r}_B_{B}_mu_{mu}_K_{K}.tsv"
    ...
    outname = f"src/understandabilityVsHamming2D/plots/latticeNbrVsGlobal/umap_clusters_L_{L}_g_{g}_a_{a}_r_{r}_B_{B}_mu_{mu}_K_{K}.png"
    plt.tight_layout()
