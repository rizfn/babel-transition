...
int main(int argc, char *argv[])
{
    ...
    for (int step = 0; step < steps; ++step)
    {
        ...
        // 1a. Local interactions (with 4 neighbors, only alpha/communicability matters)
        for (int i = 0; i < L; ++i)
        {
            for (int j = 0; j < L; ++j)
            {
        ...
        // 1b. Global interactions (random agents, only gamma/hamming matters)
    ...
    fname << exeDir << "/outputs/latticeNbrVsGlobal/L_" << L << "_g_" << gamma << "_a_" << alpha << "_r_" << globalInteractionRatio << "_B_" << B << "_mu_" << mu << "_K_" << killRadius << ".tsv";
