...
def process_file_memory_efficient(filename):
    ...
    try:
        ...
        return (gamma, alpha, sqrt_weighted_cluster_size, L, B, mu)
...
def load_cluster_data(category):
    """Load and compute cluster data for all files based on category."""
...
def create_cluster_heatmap(cluster_data, category):
    ...
    df = pd.DataFrame(cluster_data, columns=['gamma', 'alpha', 'sqrt_weighted_cluster_size', 'L', 'B', 'mu'])
    ...
    ax.set_title(f'√(E[Cluster Size²]) - {category_title}\n(L={L}, B={B}, μ={mu})')
    
    plt.tight_layout()
    
    # Save the plot
    output_dir = f"src/understandabilityVsHamming2D/globalAlphaOrK/plots/{category}/clusterSizes"
    os.makedirs(output_dir, exist_ok=True)
    fname = f"{output_dir}/heatmap_sqrtWeightedClusterSize_L_{L}_B_{B}_mu_{mu}.png"
    ...
    csv_fname = f"{output_dir}/sqrt_weighted_cluster_size_results_L_{L}_B_{B}_mu_{mu}.csv"
...
def main(category):
    ...
    # Load and compute cluster data from all files
    cluster_data = load_cluster_data(category)
    ...
    # Create the heatmap
    create_cluster_heatmap(cluster_data, category)
