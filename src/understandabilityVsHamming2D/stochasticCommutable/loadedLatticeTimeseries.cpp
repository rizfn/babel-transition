...
// Loads the last lattice from a timeseries file
std::vector<std::vector<Agent>> load_last_lattice(const std::string &filepath, int L, int B) {
    ...
    // Find the tab after the step number
    auto tab_pos = last_line.find('\t');
    std::string lattice_str = last_line.substr(tab_pos + 1);

    std::vector<std::vector<Agent>> lattice(L, std::vector<Agent>(L, Agent(B)));
    size_t i = 0, j = 0, b = 0;
    size_t pos = 0;
    while (i < L && pos < lattice_str.size()) {
        j = 0;
        while (j < L && pos < lattice_str.size()) {
            b = 0;
            while (b < B && pos < lattice_str.size() && (lattice_str[pos] == '0' || lattice_str[pos] == '1')) {
                lattice[i][j].language[b] = lattice_str[pos] - '0';
                ++b; ++pos;
            }
            // skip comma between cells
            if (pos < lattice_str.size() && lattice_str[pos] == ',') ++pos;
            ++j;
        }
        // skip semicolon between rows
        if (pos < lattice_str.size() && lattice_str[pos] == ';') ++pos;
        ++i;
    }
    return lattice;
...
// Utility: extract L and B from filename
void parse_L_B_from_filename(const std::string& filename, int& L, int& B) {
    // Example: .../L_256_B_16/g_1_a_1_mu_0.0001.tsv
    auto Lpos = filename.find("L_");
    auto Bpos = filename.find("_B_", Lpos);
    if (Lpos == std::string::npos || Bpos == std::string::npos) {
        throw std::runtime_error("Could not parse L and B from filename: " + filename);
    }
    L = std::stoi(filename.substr(Lpos + 2, Bpos - (Lpos + 2)));
    auto Bend = filename.find("/", Bpos + 3);
    if (Bend == std::string::npos) Bend = filename.find("_", Bpos + 3);
    if (Bend == std::string::npos) Bend = filename.size();
    B = std::stoi(filename.substr(Bpos + 3, Bend - (Bpos + 3)));
}

void run(std::vector<std::vector<Agent>> &lattice, double gamma, double alpha, double mu, int steps, int steps_to_record, int recording_skip, const std::string &output_path)
{
    int L = lattice.size();
    int B = lattice[0][0].language.size();
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2) gamma = std::stod(argv[2]);
    if (argc > 3) alpha = std::stod(argv[3]);
    if (argc > 4) mu = std::stod(argv[4]);
    if (argc > 5) steps_to_record = std::stoi(argv[5]);
    if (argc > 6) recording_skip = std::stoi(argv[6]);

    std::string exeDir = std::filesystem::path(argv[0]).parent_path().string();

    // Input file path (hardcoded or could be made configurable)
    std::string load_path = exeDir + "/outputs/latticeTimeseries/rasterscanMu/L_256_B_16/g_1_a_1_mu_0.0001.tsv";

    // Parse L and B from filename
    int L, B;
    parse_L_B_from_filename(load_path, L, B);

    // Load initial lattice from file
    std::vector<std::vector<Agent>> lattice = load_last_lattice(load_path, L, B);

    // Save to new folder: outputs/latticeTimeseries/loaded/L_XXX_B_YYY/g_...
    std::ostringstream fname;
    fname << exeDir << "/outputs/latticeTimeseries/loaded/L_" << L << "_B_" << B << "/g_" << gamma << "_a_" << alpha << "_mu_" << mu << ".tsv";

    run(lattice, gamma, alpha, mu, steps, steps_to_record, recording_skip, fname.str());
