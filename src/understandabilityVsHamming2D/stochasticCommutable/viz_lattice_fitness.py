...
def plot_lattice_and_fitness(lattice, step, gamma, alpha, outdir, color_map=None):
    ...
    outname = os.path.join(outdir, f"fitness_frame_{step:06d}.png")
    plt.savefig(outname, dpi=150, bbox_inches='tight')
    plt.close()

def process_lattice_fitness(L, B, gamma, alpha, mu, subdir="long"):
    ...
    outdir = os.path.join(base_dir, f"plots/latticeFitness/{subdir}/frames/L_{L}_g_{gamma}_a_{alpha}_B_{B}_mu_{mu}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gamma", type=int, default=1)  # Changed to int to match your files
    parser.add_argument("--alpha", type=float, default=1.2)  # Changed to match your files
    parser.add_argument("--mu", type=float, default=0.001)  # Changed to match your files
    parser.add_argument("--subdir", type=str, default="rasterscanMu", help="Subdirectory (e.g., 'long', 'rasterscanMu')")
    args = parser.parse_args()

    process_lattice_fitness(args.L, args.B, args.gamma, args.alpha, args.mu, args.subdir)