...
void run(int L, int B, double gamma, double alpha, double mu, int steps, int steps_to_record, int recording_skip, const std::string &output_path)
{
    ...
    for (int step = 0; step < steps; ++step)
    {
        ...
        // For the last steps_to_record steps, record cluster sizes
...
int main(int argc, char *argv[])
{
    ...
    fname << exeDir << "/outputs/clusterTimeseries/constantGamma/L_" << L << "_g_" << gamma << "_a_" << alpha << "_B_" << B << "_mu_" << mu << ".tsv";
