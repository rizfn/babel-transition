...
def plot_lattice_and_heatmap(lattice, step, outdir, color_map=None):
    ...
    outname = os.path.join(outdir, f"frame_{step:06d}.png")
    plt.savefig(outname, dpi=150, bbox_inches='tight')
    plt.close()

def process_lattice_timeseries(L, B, gamma, alpha, mu, subdir="long"):
    ...
    outdir = os.path.join(base_dir, f"plots/latticeAnim/{subdir}/frames/L_{L}_g_{gamma}_a_{alpha}_B_{B}_mu_{mu}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", type=int, default=0.8)  # Changed to int to match your files
    parser.add_argument("--mu", type=float, default=0.005)  # Changed to match your files
