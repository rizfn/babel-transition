...
for gamma in "${GAMMA_VALUES[@]}"; do
    for alpha in "${ALPHA_VALUES[@]}"; do
        JOBLIST+=("$L $B $N_STEPS $gamma $alpha $MU $STEPS_TO_RECORD $RECORDING_SKIP")
    done
done

TOTAL_JOBS=${#JOBLIST[@]}
echo "Total jobs to run: $TOTAL_JOBS"
echo "Parameters: L=$L, B=$B, N_STEPS=$N_STEPS, MU=$MU"
...
run_job() {
    ...
    $EXE "$L" "$B" "$N_steps" "$gamma" "$alpha" "$mu" "$steps_to_record" "$recording_skip"
