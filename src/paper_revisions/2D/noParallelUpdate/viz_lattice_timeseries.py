...
def process_lattice_timeseries(L, B, gamma, alpha, mu):
    """Process lattice timeseries from the specified subdirectory."""
    # Get the current directory (stochasticCommutable)
    base_dir = os.path.dirname(__file__)
    filename = os.path.join(base_dir, f"outputs/latticeTimeseries/L_{L}_B_{B}/g_{gamma}_a_{alpha}_mu_{mu}.tsv")
...
if __name__ == "__main__":
    ...
    process_lattice_timeseries(L, B, gamma, alpha, mu)