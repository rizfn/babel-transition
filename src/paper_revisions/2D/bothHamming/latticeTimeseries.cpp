...
double hamming_distance(const std::vector<int> &a, const std::vector<int> &b, int B)
{
    int count = 0;
    for (int i = 0; i < B; ++i)
        count += (a[i] != b[i]);
    return static_cast<double>(count) / static_cast<double>(B);
}
...
void update(std::vector<std::vector<Agent>> &lattice, double gamma, double alpha, double mu, int L, int B)
{
    ...
    for (int i = 0; i < L; ++i)
    {
        for (int j = 0; j < L; ++j)
        {
            ...
            // 2a. Global interaction with mean field: gamma * dH
            double global_fitness = gamma * mean_field_distance(agent.language, mean_field, B);
            agent.fitness += global_fitness;
            
            // 2b. Local interactions with neighbors: alpha * (1 - dH) summed over neighbors
            ...
            for (int d = 0; d < 4; ++d)
            {
                ...
                double dH = hamming_distance(agent.language, neighbor.language, B);
                local_fitness += alpha * (1.0 - dH);
            }
...
int main(int argc, char *argv[])
{
    ...
    fname << exeDir << "/outputs/rasterscanMu/L_" << L << "_B_" << B << "/g_" << gamma << "_a_" << alpha << "_mu_" << mu << ".tsv";
