                     ...
                     const std::string &fitness_file, const std::string &languages_file)
{
    ...
    for (int generation = 0; generation <= generations; ++generation)
    {
        ...
        std::cout << "Progress: " << std::fixed << std::setprecision(2) << (static_cast<double>(generation + 1) / generations * 100.0) << "%\r" << std::flush;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2)
        alpha = std::stod(argv[2]); // New parameter for genetic similarity
    if (argc > 3)
        N = std::stoi(argv[3]);
    if (argc > 4)
        L = std::stoi(argv[4]);
    ...
    // Run evolution with genetic similarity
