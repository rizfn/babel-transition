...
constexpr double DEFAULT_ALPHA = 10.0;
    ...
    for (int generation = 0; generation <= generations; ++generation)
    {
        ...
        for (int round = 0; round < N_rounds; ++round)
        {
            ...
            for (int i = 0; i < N; i += 2)
            {
                ...
                double fit_a = sumBits(a.language) / L - gamma * hamming(a.language, b.language) / L + genetic_bonus;
                double fit_b = sumBits(b.language) / L - gamma * hamming(b.language, a.language) / L + genetic_bonus;
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 6)
        mu = std::stod(argv[6]);
    if (argc > 7)
        beta = std::stod(argv[7]); // Changed from children_per_success to beta
    if (argc > 8)
