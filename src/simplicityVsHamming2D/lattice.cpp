...
constexpr double DEFAULT_ALPHA = 0.0;
...
int main(int argc, char *argv[])
{
        ...
        for (int i = 0; i < L; ++i)
        {
            for (int j = 0; j < L; ++j)
            {
                ...
                for (int d = 0; d < 4; ++d)
                {
                    ...
                    double fit = alpha * (double(sumBits(a.language)) / B) + gamma * (double(d_hamming) / B);
                    a.fitness += fit;
                }
    ...
    fname << exeDir << "/outputs/lattice/L_" << L << "_g_" << gamma << "_a_" << alpha << "_mu_" << mu << "_K_" << killRadius << ".tsv";
