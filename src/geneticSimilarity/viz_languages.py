...
def umap_clusters_beta2params(gamma, alpha, N, L, mu):
    """Run UMAP first, then clustering with epsilon adjusted by diversity."""
    # Load language data
    base_dir = "src/geneticSimilarity/outputs/beta/languages"
    ...
    output_file = f"{output_dir}/beta_umap_clusters_g_{gamma}_a_{alpha}_N_{N}_L_{L}{gen_suffix}.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Visualization saved to: {output_file}")
    plt.show()

def umap_clusters_betaNegative(gamma, N, L, mu, max_depth):
    """Run UMAP first, then clustering with epsilon adjusted by diversity."""
    # Load language data
    base_dir = "src/geneticSimilarity/outputs/betaNegative/languages"
    languages_file = f"{base_dir}/g_{gamma}_N_{N}_L_{L}_mu_{mu}_gdmax_{max_depth}.tsv"
    ...
    gen_suffix = f"_gen_{last_gen}" if 'last_gen' in locals() else ""
    output_file = f"{output_dir}/betaNegative_umap_clusters_g_{gamma}_N_{N}_L_{L}_mu_{mu}_gdmax_{max_depth}{gen_suffix}.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Visualization saved to: {output_file}")
    plt.show()
...
if __name__ == "__main__":
    ...
    # umap_clusters_beta2params(0, 1000, 1000, 16, 0.01)
    umap_clusters_betaNegative(100, 1000, 16, 0.01, 1000)
