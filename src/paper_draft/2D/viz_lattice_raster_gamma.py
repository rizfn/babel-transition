...
def load_final_lattices_alpha_gamma_raster(L, B, mu):
    """Load the final lattice from each file matching the specified parameters from rasterscan folder."""
    ...
    for filename in tqdm(files, desc="Loading final lattices"):
        gamma, alpha, mu_file = extract_params_from_filename(filename)
        if gamma is None or alpha is None or mu_file is None:
            continue
        if mu_file != mu:
            continue
        try:
...
def create_lattice_alpha_gamma_raster_plot(lattice_data, L, B, mu):
    """Create a grid plot of lattices organized by gamma (x-axis) and alpha (y-axis) with clear axes and cell borders."""
    if not lattice_data:
        print("No lattice data to plot")
        return
    alphas = sorted(set(key[0] for key in lattice_data.keys()), reverse=True)  # High to low
    gammas = sorted(set(key[1] for key in lattice_data.keys()))
    ...
    for i, alpha in enumerate(alphas):
        for j, gamma in enumerate(gammas):
            ...
            if (alpha, gamma) in lattice_data:
                data = lattice_data[(alpha, gamma)]
...
def main_alpha_gamma_raster(L=256, B=16, mu=0.0001):
    ...
    lattice_data = load_final_lattices_alpha_gamma_raster(L, B, mu)
    if not lattice_data:
