...
constexpr double DEFAULT_ALPHA = 0;
constexpr double DEFAULT_MU = 0.0001;
constexpr int DEFAULT_STEPS_TO_RECORD = 100000;
...
void run(int L, int B, double gamma, double alpha, double mu, int steps, int steps_to_record, int recording_skip, const std::string &output_path)
{
    ...
    for (int step = 0; step < steps; ++step)
    {
        ...
        // For the last steps_to_record steps, record population counts
        if ((step >= steps - steps_to_record) && (step % recording_skip == 0))
        {
            // Count bitstring populations
            std::map<std::string, int> population_counts;

            for (int i = 0; i < L; ++i)
            {
                for (int j = 0; j < L; ++j)
                {
                    // Convert language vector to string
                    std::string bitstring = "";
                    for (int b = 0; b < B; ++b)
                        bitstring += std::to_string(lattice[i][j].language[b]);

                    population_counts[bitstring]++;
                }
            }

            // Write step number and population counts
            fout << step << "\t";
            bool first = true;
            for (const auto &[bitstring, count] : population_counts)
            {
                if (!first)
                    fout << "\t";
                fout << bitstring << "," << count;
                first = false;
            }
            fout << "\n";
        }
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 6)
        mu = std::stod(argv[6]);
    if (argc > 7)
        steps_to_record = std::stoi(argv[7]);
    if (argc > 8)
        recording_skip = std::stoi(argv[8]);

    std::string exeDir = std::filesystem::path(argv[0]).parent_path().string();
    std::ostringstream fname;
    fname << exeDir << "/outputs/populationTimeseries/L_" << L << "_B_" << B << "/g_" << gamma << "_a_" << alpha << "_mu_" << mu << ".tsv";
