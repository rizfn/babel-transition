...
// Get largest cluster size fraction
double get_largest_cluster_fraction(const std::vector<std::vector<Agent>>& lattice, int L)
{
    ...
    // For each unique language, find its largest cluster
    for (const auto& [lang_str, positions] : language_positions)
    {
        ...
        // Count cluster sizes and find maximum
    ...
    return static_cast<double>(max_cluster_size) / static_cast<double>(L * L);
}
...
void run(int L, int B, double gamma, double alpha, double mu, int steps, int steps_to_record, int recording_skip, const std::string &output_path)
{
    ...
    // Write header
    fout << "step\tlargest_cluster_fraction\n";
    ...
    for (int step = 0; step < steps; ++step)
    {
        ...
        // For the last steps_to_record steps, record the largest cluster fraction
        if ((step >= steps - steps_to_record) && (step % recording_skip == 0))
        {
            double largest_cluster_fraction = get_largest_cluster_fraction(lattice, L);
            fout << step << "\t" << std::fixed << std::setprecision(6) << largest_cluster_fraction << "\n";
        }
    }
    fout.close();
}
...
int main(int argc, char *argv[])
{
    ...
    fname << exeDir << "/outputs/rasterscanOrderParam/L_" << L << "_B_" << B << "/g_" << gamma << "_a_" << alpha << "_mu_" << mu << "_" << timestamp << ".tsv";
