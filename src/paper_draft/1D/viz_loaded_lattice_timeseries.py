...
def _plot_single_seed(L, B, seed):
    ...
    # Parse parameters from filenames and sort
    file_params = []
    ...
    for idx, (gamma, alpha, mu, filepath) in enumerate(file_params):
        ...
        # Only add labels to the leftmost plot
        if idx == 0:
    ...
    output_dir = script_dir / "plots" / "latticeTimeseriesLoaded" / f"L_{L}_B_{B}"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"all_loaded_comparison_seed_{seed}.svg"
...
if __name__ == "__main__":
    plot_all_loaded_lattices(512, 16, seed=np.arange(60, 70))
