    ...
    for (int generation = 0; generation <= generations; ++generation)
    {
        ...
        for (int round = 0; round < N_rounds; ++round)
        {
            ...
            for (int i = 0; i < N; i += 2)
            {
                ...
                double fit_a, fit_b;
                if (d_genetic > 0) {
                    // Related: bonus for simple language, scaled by genetic closeness, plus unrelated bonus
                    double sum_a = static_cast<double>(sumBits(a.language));
                    double sum_b = static_cast<double>(sumBits(b.language));
                    double closeness = 1.0 - (static_cast<double>(d_genetic) / static_cast<double>(max_depth));
                    fit_a = alpha * (sum_a / L) * closeness + gamma * (static_cast<double>(d_hamming) / L);
                    fit_b = alpha * (sum_b / L) * closeness + gamma * (static_cast<double>(d_hamming) / L);
                } else {
...
int main(int argc, char *argv[])
{
    ...
    fitness_stream << exeDir << "/outputs/beta/fitness/g_" << gamma
                   << "_a_" << alpha << "_gdmax_" << max_depth << "_N_" << N 
