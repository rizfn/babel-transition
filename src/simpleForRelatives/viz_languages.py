...
def umap_clusters_beta(gamma, alpha, max_depth, N, beta, L, mu):
    """Run UMAP first, then clustering with epsilon adjusted by diversity."""
    # Load language data
    base_dir = "src/simpleForRelatives/outputs/beta/languages"
    languages_file = f"{base_dir}/g_{gamma}_a_{alpha}_gdmax_{max_depth}_N_{N}_b_{beta}_L_{L}_mu_{mu}.tsv"
    ...
    output_file = f"{output_dir}/beta_{beta}_umap_clusters_g_{gamma}_a_{alpha}_gdmax_{max_depth}_N_{N}_L_{L}_mu_{mu}{gen_suffix}.png"
...
if __name__ == "__main__":
    umap_clusters_beta(0, 100, 20, 1000, 0.1, 16, 0.01)