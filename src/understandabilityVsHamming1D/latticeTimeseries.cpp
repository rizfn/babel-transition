...
int find_weakest_neighbor(const std::vector<Agent> &lattice, int cx, int L)
{
    double min_fitness = std::numeric_limits<double>::infinity();
    std::vector<int> weakest_neighbors;
    
    // Check both neighbors in 1D (left and right with periodic boundary)
    int neighbors[2] = {(cx - 1 + L) % L, (cx + 1) % L};
    
    // First pass: find minimum fitness among non-immune neighbors
    for (int d = 0; d < 2; ++d)
    {
        int nx = neighbors[d];
        if (!lattice[nx].immune)
        {
            if (lattice[nx].fitness < min_fitness)
            {
                min_fitness = lattice[nx].fitness;
            }
        }
    }
    ...
    {
        int nx = neighbors[d];
        if (!lattice[nx].immune && lattice[nx].fitness == min_fitness)
        {
            weakest_neighbors.push_back(nx);
        }
    }
...
void update(std::vector<Agent> &lattice, double gamma, double alpha, double mu, int L, int B)
{
    ...
    for (int i = 0; i < L; ++i)
    {
        for (int b = 0; b < B; ++b)
        {
            mean_field[b] += static_cast<double>(lattice[i].language[b]);
        }
    }
    ...
    for (int i = 0; i < L; ++i)
    {
        ...
        // 2b. Local interactions with neighbors (left and right)
        double local_fitness = 0.0;
        int neighbors[2] = {(i - 1 + L) % L, (i + 1) % L};
        
        for (int d = 0; d < 2; ++d)
        {
            Agent &neighbor = lattice[neighbors[d]];
            int comm = communicability(agent.language, neighbor.language, B);
            local_fitness += (alpha / 2.0) * (static_cast<double>(comm) / static_cast<double>(B));
        }
    ...
    // Reset immunity
    for (auto &agent : lattice)
        agent.immune = false;

    // Perform L/2 invasion trials
    int trials = L / 2;
    for (int trial = 0; trial < trials && trial < positions.size(); ++trial)
    {
        int i = positions[trial];
        ...
        int wi = find_weakest_neighbor(lattice, i, L);
        ...
        if (lattice[i].fitness > lattice[wi].fitness)
        {
            // Invade: clone current agent into weakest neighbor position
            lattice[wi].language = lattice[i].language;
            lattice[i].immune = true;
            lattice[wi].immune = true;
        }
    }
    ...
    for (int i = 0; i < L; ++i)
    {
        ...
        {
            if (dis(gen) < mu)
                lattice[i].language[b] = 1 - lattice[i].language[b];
        }
    }
}
...
void run(int L, int B, double gamma, double alpha, double mu, int steps, int steps_to_record, int recording_skip, const std::string &output_path)
{
    ...
    std::vector<Agent> lattice(L, Agent(B));
    for (int i = 0; i < L; ++i)
        lattice[i].language = std::vector<int>(B, 0);
